...
app.controller("planningCtrl", function ($scope, $location, $http, modalService, $routeParams) {
    ...
    $scope.loadMonth = function(){
        ...
        $http({
            method : "GET",
            url : "control.reservering.php?action=planning&start=" + moment(dStart).format(sSvrF) + "&eind=" + moment(dEind).format(sSvrF)
    ...
    $scope.openRes = function(sId){
        $location.path('/reservering/' + sId);
    }
