<?
    ...
    if($_GET['action'] == "overview" && $_SERVER['REQUEST_METHOD'] == "GET"){
        ...
        if($_GET['uitgeschakeld'] == "true"){
            ...
            $historie = "WHERE actief=1 AND (einddatum IS NULL OR einddatum >= NOW())";
        }
        ...
        $response = SQL_Query_Array("SELECT id, groep_naam, startdatum, einddatum, ochtend, middag, avond, herhaaltype, herhaalweekdag, herhaalweek, herhaaldag, actief FROM herhaling $historie ORDER BY $sort;");
    }    
    ...
    if($_GET['action'] == "save" && $_SERVER['REQUEST_METHOD'] == "POST"){
        ...
        $middag             = SQL_Filter($request['middag'], 'bool');
        $ochtend            = SQL_Filter($request['ochtend'], 'bool');
        $avond              = SQL_Filter($request['avond'], 'bool');
        $actief             = SQL_Filter($request['actief'], 'bool');
        ...
        if($continue){
            ...
            if($request['id'] == "new"){
                ...
                if(SQL_Query_Exec("INSERT INTO herhaling(groep_naam, startdatum, einddatum, ochtend, middag, avond, herhaaltype, herhaalweekdag, herhaalweek, herhaaldag, actief) VALUES ($groep_naam, $startdatum, $einddatum, $ochtend, $middag, $avond, $herhaaltype, $herhaalweekdag, $herhaalweek, $herhaaldag, $actief)")){
                    ...
                    $response['data'] = SQL_Query_Array("SELECT * FROM herhaling WHERE id=$id;");
                }
            }else{
